# RAG 연결 + LLM_MODEL 기반 provider 분기
...
# =========================
# .env 파일에서 환경변수를 읽어온다
# [요리] 비유: 비법 소스 보관함을 여는 단계
load_dotenv()

# mock mode 설정: .env의 MOCK_MODE=true 이면 실제 LLM API 호출X
# [요리] 비유: 진짜 셰프를 부르지 않고 시식용 샘플 응답만 내는 상태
...
def get_provider_and_model(model_name: str) -> tuple[str, str]:
    ...
    [요리] 비유:
    주문서에 적힌 셰프 이름을 보고
    Gemini 셰프, Mistral 셰프, HuggingFace 셰프 중
    누구에게 보낼지 정합니다.
    """
